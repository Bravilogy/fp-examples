 ...
 * with traverse, Task and all kinds of
            ...
            <div className='content'>
                <Problem>
                    ...
                    <p>
                        ...
                        the ID prop from the <strong>JSON</strong> response (<em>for example's sake</em>), <strong>then</strong> get all posts by that user and while we're at it, we need to
                ...
                <Solution>
                    <p>
                        Let's use an awesome <code>Task</code> monad here. First, we wrap our <code>fetch</code> in <code>Task</code>:
                    </p>
...
const getUserWithPosts = id => getUser(id)
    // now that we have the user, we chain to another Task, in this case - getPosts
    .chain(user => getPosts(user.id)
        // and now for each post, we need to return a Task, but we don't want to end up with
        // [ Task response, Task response, Task response, ... ] but rather
        // Task([ response, response, response, ... ]). So we need to
        // flip the types around so to say. This is where traverse
        // comes in and since traverse will return us another
        // Task, we can chain to it straight away:
...
const getComments = postId =>
    httpGet(\`http://jsonplaceholder.typicode.com/comments?postId=\${postId}\`);`}
                    ...
                    <p>
                        Looking at our previous function, two things are clear - we need a function that will <code>dissoc</code> (remove)
                        specific key from the list of things, be it <strong>posts</strong> or <strong>comments</strong> and at the same time, it should
    ...
    lift(compose(objOf(withKey), map(dissoc(withoutKey))));`}
                    ...
                    <p>
                        ...
                        it with another resource, for example - a <strong>user</strong> with <strong>posts</strong> and a single <strong>post</strong> with <strong>comments</strong>. It will do so by using the function above:
                    </p>
                    <Highlight className='javascript'>
...
const userWithPosts = mergeWithResource('posts', 'userId', compose(chain(postWithComments), getPosts));`}
                    ...
                    <p>
                        And the final line, our main composition with <code>composeK</code> since we're dealing with a monad here:
                    </p>
                    <Highlight className='javascript'>
